...
void LocalPlannerVisualization::initializePublishers(const rclcpp::Node::SharedPtr nh) {
  node_ = nh;
  // local_pointcloud_pub_ = node_->create_publisher<pcl::PointCloud<pcl::PointXYZ>>
  //   ("/local_pointcloud", rclcpp::SystemDefaultsQoS());
  pointcloud_size_pub_ = node_->create_publisher<std_msgs::msg::UInt32>("/pointcloud_size", 1);
  bounding_box_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/bounding_box", 1);
  ground_measurement_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/ground_measurement", 1);
  original_wp_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/original_waypoint", 1);
  adapted_wp_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/adapted_waypoint", 1);
  smoothed_wp_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/smoothed_waypoint", 1);
  complete_tree_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/complete_tree", 1);
  tree_path_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/tree_path", 1);
  marker_goal_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/goal_position", 1);
  path_actual_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/path_actual", 1);
  path_waypoint_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/path_waypoint", 1);
  path_adapted_waypoint_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/path_adapted_waypoint", 1);
  current_waypoint_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/current_setpoint", 1);
  histogram_image_pub_ = node_->create_publisher<sensor_msgs::msg::Image>("/histogram_image", 1);
  cost_image_pub_ = node_->create_publisher<sensor_msgs::msg::Image>("/cost_image", 1);
  closest_point_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/closest_point", 1);
  deg60_point_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/deg60_point", 1);
  fov_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/fov", 4);
  range_scan_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/range_scan", 1);
}
...
void LocalPlannerVisualization::visualizePlannerData(const LocalPlanner& planner,
  ...
  // visualize clouds
  // TODO: Enable local pointcloud pub
...
void LocalPlannerVisualization::publishDataImages(const std::vector<uint8_t>& histogram_image_data,
  ...
  cost_img.header.stamp = node_->get_clock()->now();
  ...
  hist_img.header.stamp = node_->get_clock()->now();
...
void LocalPlannerVisualization::publishPaths(const Eigen::Vector3f& last_position,
  ...
  path_actual_marker.header.stamp = node_->get_clock()->now();
  ...
  path_waypoint_marker.header.stamp = node_->get_clock()->now();
  ...
  path_adapted_waypoint_marker.header.stamp = node_->get_clock()->now();
...
void LocalPlannerVisualization::publishCurrentSetpoint(const geometry_msgs::msg::Twist& wp,
  ...
  setpoint.header.stamp = node_->get_clock()->now();
  setpoint.id = 0;
